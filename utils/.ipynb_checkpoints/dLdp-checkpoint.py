...
def analytic_dLdp(q,ps,C1s,C0s,ks,bs,FIM=None):
    ...
    if FIM is None:
        ...
        Q = np.linalg.inv(wrhat.T.dot(wrhat) + delta * np.eye(2) ) # Default calculation of FIM^-1
    else:
        # print('Coordinating')
...
def dhdz(x,ps,C1s,C0s,ks,bs):

    q = x.flatten()
    q = q[:len(q)//2]
    dh = dhdq(q,ps,C1s,C0s,ks,bs)
    return jnp.hstack([dh,np.zeros(dh.shape)])
   
def dhdq(q,ps,C1s,C0s,ks,bs):
    rs = jnp.linalg.norm(ps-q,axis=1)
