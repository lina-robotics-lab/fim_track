...
def launch_tracking_suite(pose_type_string,n_robots,local_track_alg,sensor_names):
	...
	rospy.init_node('test_waypoint_tracking', anonymous=False)
	...
	args=" ".join([pose_type_string])
	node=roslaunch.core.Node(package='fim_track',node_type='simple_waypoint_generator.py',name='multi_robot_controller',namespace='/',args=args,output='screen')
	launch.launch(node)
